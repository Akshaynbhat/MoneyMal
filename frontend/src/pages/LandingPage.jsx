...
export default function LandingPage() {
    ...
    // Preview modal state
    const [previewing, setPreviewing] = useState(false);   // loading preview from API
    ...
    const handleFile = useCallback(async (f) => {
        ...
        // Immediately fetch the column-mapping preview
        setPreviewing(true);
        try {
            ...
            showToast('Could not fetch column preview — you can still run analysis.', 'warning');
        } finally {
            setPreviewing(false);
        }
    }, [setFile, showToast]);

    // Triggered from the modal "Confirm & Run Detection" button
    ...
    // Direct "Launch Analysis" button (re-opens modal if mapping data exists, else runs directly)
    ...
    }, [file, mappingData, analyze, navigate]);
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        {previewing ? (
                            <div>
                                ...
                                <p style={{ fontFamily: 'var(--font-mono)', color: 'var(--color-accent)', fontSize: '0.85rem' }}>
                            ...
                            <div>
                                ...
                                <p style={{ color: 'var(--color-text-dim)', fontSize: '0.75rem', marginTop: '4px', fontFamily: 'var(--font-mono)' }}>
                                    {(file.size / 1024).toFixed(1)} KB — Click below to review mapping &amp; run detection
                                </p>
                            </div>
                        ) : (
                            <div>
                                ...
                                <p style={{ color: 'var(--color-text-dim)', fontFamily: 'var(--font-mono)', fontSize: '0.7rem', marginTop: '10px' }}>
                                    Flexible column detection — any naming convention accepted
                                </p>
                            </div>
                        )}
                    </div>
                    ...
                    <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                    >
                        {loading ? '◌ ANALYZING...' : previewing ? '◌ LOADING PREVIEW...' : '▶ LAUNCH ANALYSIS'}
                    </motion.button>
                </motion.div>
