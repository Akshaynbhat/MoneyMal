...
app = FastAPI(
    title="MoneyMal",
    description="Money Muling Detection Engine",
    version="5.0.0",
)
...
async def preview_mapping(file: UploadFile = File(...)):
    ...
    try:
        ...
        result = ingestor.normalize_dataframe(df)
        return JSONResponse(content={
            "mapping_summary": result["mapping_summary"],
            "mapped_canonicals": result.get("mapped_canonicals", []),
...
async def analyze_private(file: UploadFile = File(...), current_user: dict = Depends(get_analyst_or_admin)):
    ...
    try:
        ...
        df = pd.read_csv(io.BytesIO(content))
        ingestor = DataIngestor()
        norm_result = ingestor.normalize_dataframe(df)
        df = norm_result["df"]
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Failed to parse CSV: {e}")
