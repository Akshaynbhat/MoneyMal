...
from account_type_filter import load_threshold_config, apply_account_type_filter
...
class ForensicsEngine:
    ...
    FLAG_THRESHOLD = 40     # Minimum score to be flagged
    ...
    def __init__(self):
        ...
        # Account-type pre-filter state
        self._threshold_config: dict = load_threshold_config()
        self._account_type_flag_reasons: dict[str, list[str]] = {}
    ...
    def load_data(self, df: pd.DataFrame) -> None:
        ...
        # ── Account-type pre-filter (runs BEFORE ML / Isolation Forest) ── #
        df, self._account_type_flag_reasons = apply_account_type_filter(
            df, self._threshold_config
        )
    ...
    def detect_shells(self) -> None:
        ...
        # Build per-account first/last seen — vectorized
        s_first = self.df.groupby("sender_id")["timestamp"].min()
        r_first = self.df.groupby("receiver_id")["timestamp"].min()
        s_last  = self.df.groupby("sender_id")["timestamp"].max()
        r_last  = self.df.groupby("receiver_id")["timestamp"].max()
        all_accs_seen = set(s_first.index) | set(r_first.index)
        account_first_seen = {}
        account_last_seen  = {}
        for _acc in all_accs_seen:
            cands_f = [s_first[_acc]] if _acc in s_first.index else []
            cands_f += [r_first[_acc]] if _acc in r_first.index else []
            cands_l = [s_last[_acc]]  if _acc in s_last.index  else []
            cands_l += [r_last[_acc]] if _acc in r_last.index  else []
            if cands_f: account_first_seen[_acc] = min(cands_f)
            if cands_l: account_last_seen[_acc]  = max(cands_l)
    ...
    def detect_velocity(self) -> None:
        ...
        # Build per-account std/mean using groupby then apply CV
        amt_stats_s = self.df.groupby("sender_id")["amount"].agg(["mean", "std", "count"])
        amt_stats_r = self.df.groupby("receiver_id")["amount"].agg(["mean", "std", "count"])
        all_acc_lv  = set(amt_stats_s.index) | set(amt_stats_r.index)
        account_amounts: dict[str, list[float]] = defaultdict(list)
        for acc_lv in all_acc_lv:
            vals = []
            if acc_lv in amt_stats_s.index:
                row_s = amt_stats_s.loc[acc_lv]
                if row_s["count"] >= 2 and row_s["mean"] > 0:
                    vals.append((row_s["std"] / row_s["mean"], int(row_s["count"])))
            if acc_lv in amt_stats_r.index:
                row_r = amt_stats_r.loc[acc_lv]
                if row_r["count"] >= 2 and row_r["mean"] > 0:
                    vals.append((row_r["std"] / row_r["mean"], int(row_r["count"])))
            if vals:
                # weighted mean CV
                total_n  = sum(v[1] for v in vals)
                mean_cv  = sum(v[0] * v[1] for v in vals) / total_n if total_n > 0 else 1.0
                if mean_cv < 0.2:
                    self._low_variance_accounts.add(acc_lv)
                    self.account_patterns[acc_lv].add("low_variance")
        account_amounts = defaultdict(list)  # keep for hub detection below
        ...
        if dataset_span > 0:
            # Build account_timestamps vectorized
            ts_s = self.df[["sender_id", "timestamp"]].rename(columns={"sender_id": "account"})
            ts_r = self.df[["receiver_id", "timestamp"]].rename(columns={"receiver_id": "account"})
            ts_all = pd.concat([ts_s, ts_r], ignore_index=True)
            account_timestamps: dict[str, list] = {
                acc: list(grp["timestamp"].values)
                for acc, grp in ts_all.groupby("account")
            }
    ...
    def calculate_suspicion_scores(self) -> None:
        ...
        # Build cycle account sets from detected rings (injected into flags — avoids per-account DFS)
        cycle_accounts:       set[str] = set()
        long_cycle_accounts:  set[str] = set()
        for ring in self.fraud_rings:
            if ring.get("pattern_type") == "cycle":
                cycle_accounts.update(ring["member_accounts"])
        # Long cycles (length >= 4) from account_patterns
        for acc, patterns in self.account_patterns.items():
            for pat in patterns:
                if pat in ("cycle_length_4", "cycle_length_5"):
                    long_cycle_accounts.add(acc)
                    cycle_accounts.add(acc)

        # 1. Run RBI Rules — pass pre-computed cycle sets (no per-account DFS)
        nodes = list(self.G.nodes())
        flags_by_acc = run_all_flags(
            self.df, self.G, nodes,
            cycle_accounts=cycle_accounts,
            long_cycle_accounts=long_cycle_accounts,
        )

        # Merge account-type pre-filter reasons into flags_by_acc
        # so ML pipeline sees pre-flagged accounts
        for acc, reasons in self._account_type_flag_reasons.items():
            if acc not in flags_by_acc:
                flags_by_acc[acc] = []
            # Encode each breach as a synthetic flag token "AT_BREACH"
            if reasons and "AT_BREACH" not in flags_by_acc[acc]:
                flags_by_acc[acc].append("AT_BREACH")
    ...
    def generate_json(self) -> dict:
        ...
        for acc in self.G.nodes() if self.G else []:
            ...
            # Build flag_reason: account-type breach reasons take priority;
            # fall back to "ML anomaly score" if no threshold was breached.
            at_reasons = self._account_type_flag_reasons.get(acc, [])
            if at_reasons:
                flag_reason = "; ".join(at_reasons)
            else:
                ml_score_val = ml_data.get("score", score)
                flag_reason = f"ML anomaly score: {round(ml_score_val, 1)}"
            ...
            suspicious_accounts.append({
                ...
                "rule_based_fraud": bool(at_reasons),
            })
